  ...
  server: {
    handlers: {
      POST: async () => {
        ...
        for (const c of (chs ?? []) as Array<{ id: string; created_by: string | null }>) {
          ...
          const cur = byUser.get(c.created_by) ?? { count: 0, participants: 0, points: 0 };
          cur.count += 1;
          cur.participants += participants;
          cur.points += pts;
          byUser.set(c.created_by, cur);
        }
