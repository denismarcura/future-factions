  ...
  return (
    <AppShell>
      ...
      <section className="mb-8">
        ...
        <div className="flex items-center justify-center gap-3 mt-4"><button type="button" onClick={() => setHomePage((p) => Math.max(0, p - 1))} disabled={homePage === 0} className="flex items-center gap-1 px-4 py-2 rounded-full border border-border text-sm font-bold disabled:opacity-40 hover:bg-primary/10 transition"><ChevronLeft className="h-4 w-4" /> Anterior</button><span className="text-xs text-muted-foreground">P�gina {homePage + 1}</span><button type="button" onClick={() => setHomePage((p) => p + 1)} className="flex items-center gap-1 px-4 py-2 rounded-full border border-border text-sm font-bold disabled:opacity-40 hover:bg-primary/10 transition">Pr�ximo <ChevronRight className="h-4 w-4" /></button></div></div>
      </section>
